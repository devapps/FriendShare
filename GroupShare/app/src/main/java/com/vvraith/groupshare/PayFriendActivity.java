...
public class PayFriendActivity extends AppCompatActivity implements MyAppActivity {
        ...
        private static final String TAG = "PayFriendActivity";
        ProgressDialog progressDialog = null;
        private static final int REQUEST_SIGNUP = 0;
        String requestUrl = "/api/paymoney/friend?";

        @Bind(R.id.input_toname)
        EditText _tonameText;
        @Bind(R.id.input_tovirtualid)
        EditText _tovirtualidText;
        ...
        protected void onCreate(Bundle savedInstanceState) {
            ...
            setContentView(R.layout.activity_pay_friend);
            ButterKnife.bind(this);
            TransactionId_upi = String.valueOf(new Random().nextInt(99999999));
            progressDialog = new ProgressDialog(PayFriendActivity.this,
                    R.style.AppThemeDarkDialog);

            extras = getIntent().getExtras();
            _tonameText.setText(extras.getString("friendname"));
            _tovirtualidText.setText(extras.getString("friendvirtualid"));
       ...
       /* credAllowedString = "{\n" +
        ...
        public void onFailure() {
            ...
            progressDialog.dismiss();
            Toast.makeText(getBaseContext(), "Pay Transaction failed", Toast.LENGTH_LONG).show();
        ...
        public boolean validate() {
            boolean valid = true;
            String toname = _tonameText.getText().toString();
            String tovirtualid = _tovirtualidText.getText().toString();
            String amount = _amountText.getText().toString();
            String remarks = _remarksText.getText().toString();
            if (toname.isEmpty()) {
                _tovirtualidText.setError("cannot be empty");
                valid = false;
            } else {
                _tovirtualidText.setError(null);
            }
            if (tovirtualid.isEmpty() || tovirtualid.length() < 6) {
                _tovirtualidText.setError("at least 6 alphanumeric characters");
                valid = false;
            } else {
                _tovirtualidText.setError(null);
            }
        ...
        public boolean onCreateOptionsMenu(Menu menu) {
            ...
            getMenuInflater().inflate(R.menu.menu_pay_friend, menu);
            return true;
        }
        ...
        void payAmountUPI()
        {
            ...
            progressDialog.setMessage("Raising Pay Request...");
            progressDialog.show();
...
*/
                        ...
                        inputMap.put("tousername",extras.getString("tousername"));
                        inputMap.put("friendname", extras.getString("friendname"));
                        inputMap.put("amount", amount);
                        inputMap.put("myname",toname);
                        inputMap.put("remarks", remarks);
                        new RequestExecutor(inputMap,requestUrl,this,"PAY").execute();
                    } catch (JSONException e) {
                        e.printStackTrace();
                    }
